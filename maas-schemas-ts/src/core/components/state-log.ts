...
export const ObsoleteTime = t.brand(t.string, (x): x is t.Branded<string, ObsoleteTimeBrand> => ( typeof x !== 'string' || x.match(RegExp("^[0-9]+$")) !== null ), 'ObsoleteTime')
...
export const BookingStateTransition = t.brand(t.intersection([
  t.partial({
    timestamp: t.union([
      Units_.Time,
      ObsoleteTime
  ...
  ), BookingStateTransitionBrand> => true, 'BookingStateTransition')
...
export const StateLog = t.brand(t.array(BookingStateTransition), (x): x is t.Branded<Array<BookingStateTransition>, StateLogBrand> => true, 'StateLog')
