...
export const Polyline = t.brand(t.string, (x): x is t.Branded<string, PolylineBrand> => ( typeof x !== 'string' || x.match(RegExp("^([\\x5F-\\x7E]*[\\x3F-\\x5E])+$")) !== null ) && ( typeof x !== 'string' || x.length >= 3 ) && ( typeof x !== 'string' || x.length <= 65535 ), 'Polyline')
...
export const Location = t.brand(t.intersection([
  t.partial({
    lat: Latitude,
    lon: Longitude
  }),
...
export const RelaxedLocation = t.brand(t.intersection([
  t.partial({
    lat: RelaxedLatitude,
    lon: RelaxedLongitude
  }),
...
export const ShortLocationString = t.brand(t.string, (x): x is t.Branded<string, ShortLocationStringBrand> => ( typeof x !== 'string' || x.match(RegExp("^[+-]?\\d{1,3}(\\.\\d+)?,[+-]?\\d{1,3}(\\.\\d+)?$")) !== null ) && ( typeof x !== 'string' || x.length >= 1 ) && ( typeof x !== 'string' || x.length <= 64 ), 'ShortLocationString')
...
export const UnitsGeo = t.brand(t.unknown, (x): x is t.Branded<unknown, UnitsGeoBrand> => true, 'UnitsGeo')
