...
export const Id = t.brand(t.string, (x): x is t.Branded<string, IdBrand> => ( typeof x !== 'string' || x.length >= 1 ) && ( typeof x !== 'string' || x.length <= 255 ), 'Id')
...
export const PreAuthBuffer = t.brand(t.partial({
  percentageExtra: t.number,
  minimumExtra: Fare_.Fare
}), (x): x is t.Branded<{
  percentageExtra?: number,
  minimumExtra?: Fare_.Fare
}, PreAuthBufferBrand> => true, 'PreAuthBuffer')
