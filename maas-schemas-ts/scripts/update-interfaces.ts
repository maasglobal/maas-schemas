...
const patternTest = path_.join(dirSrc, globStar, globDoctest);
const patternDoctest = path_.join(dirSrc, globStar, dirTest, globTs);

const patternTypes = path_.join(dirSrc, dirTypes, globStar, globTs);
const patternUtils = path_.join(dirSrc, dirUtils, globStar, globTs);
...
for (const filepathType of types) {
  const filepathTypeGeneric = path_.relative(rootTypes, filepathType);
  const filepathOutput = path_.join(dirSrc, filepathTypeGeneric);
