/*

undefined
Response schema for autocomplete

!!! AUTO GENERATED BY IOTSFJS REFRAIN FROM MANUAL EDITING !!!
See https://www.npmjs.com/package/io-ts-from-json-schema

*/

import { NonEmptyArray } from 'fp-ts/lib/NonEmptyArray';
import * as t from 'io-ts';
import { nonEmptyArray } from 'io-ts-types/lib/nonEmptyArray';

import * as Suggestion_691d_ from './suggestion';

export type Defined = {} | null;
export class DefinedType extends t.Type<Defined> {
  readonly _tag: 'DefinedType' = 'DefinedType';
  constructor() {
    super(
      'defined',
      (u): u is Defined => typeof u !== 'undefined',
      (u, c) => (this.is(u) ? t.success(u) : t.failure(u, c)),
      t.identity,
    );
  }
}
export type DefinedC = {} & DefinedType;
export const Defined: DefinedC = new DefinedType();

export const schemaId =
  'https://schemas.maas.global/maas-backend/autocomplete/autocomplete-query/response.json';

// SuggestionsList
// The list of autocomplete suggestions
export type SuggestionsList = t.Branded<
  Array<Suggestion_691d_.Suggestion>,
  SuggestionsListBrand
>;
export type SuggestionsListC = t.BrandC<
  t.ArrayC<typeof Suggestion_691d_.Suggestion>,
  SuggestionsListBrand
>;
export const SuggestionsList: SuggestionsListC = t.brand(
  t.array(Suggestion_691d_.Suggestion),
  (x): x is t.Branded<Array<Suggestion_691d_.Suggestion>, SuggestionsListBrand> => true,
  'SuggestionsList',
);
export type SuggestionsListBrand = {
  readonly SuggestionsList: unique symbol;
};

// Response
// The default export. More information at the top.
export type Response = t.Branded<
  {
    provider?: string;
    suggestions?: SuggestionsList;
    debug?: Record<string, unknown> & Record<string, unknown>;
  } & {
    provider: Defined;
    suggestions: Defined;
  },
  ResponseBrand
>;
export type ResponseC = t.BrandC<
  t.IntersectionC<
    [
      t.PartialC<{
        provider: t.StringC;
        suggestions: typeof SuggestionsList;
        debug: t.IntersectionC<[t.UnknownRecordC, t.RecordC<t.StringC, t.UnknownC>]>;
      }>,
      t.TypeC<{
        provider: typeof Defined;
        suggestions: typeof Defined;
      }>,
    ]
  >,
  ResponseBrand
>;
export const Response: ResponseC = t.brand(
  t.intersection([
    t.partial({
      provider: t.string,
      suggestions: SuggestionsList,
      debug: t.intersection([t.UnknownRecord, t.record(t.string, t.unknown)]),
    }),
    t.type({
      provider: Defined,
      suggestions: Defined,
    }),
  ]),
  (
    x,
  ): x is t.Branded<
    {
      provider?: string;
      suggestions?: SuggestionsList;
      debug?: Record<string, unknown> & Record<string, unknown>;
    } & {
      provider: Defined;
      suggestions: Defined;
    },
    ResponseBrand
  > => true,
  'Response',
);
export type ResponseBrand = {
  readonly Response: unique symbol;
};
/** require('io-ts-validator').validator(nonEmptyArray(Response)).decodeSync(examplesResponse) // => examplesResponse */
export const examplesResponse: NonEmptyArray<Response> = [
  {
    provider: 'google',
    suggestions: [
      {
        label: 'Nyon, Switzerland',
        meta: { google: { placeId: 'ChIJZwgvdy9DjEcRR69BEhrUt28' } },
      },
      {
        label: 'Nyons, France',
        meta: { google: { placeId: 'ChIJo_XBpV-ByhIREL6_5CqrCAQ' } },
      },
    ],
  },
  {
    provider: 'routerank',
    suggestions: [
      {
        label: 'Nyon, Switzerland',
        addressId: 'rrPlaceId%3Ape--Nyon--whosonfirst%3Alocality%3A101914095',
        meta: {
          routerank: {
            id: 'pe--Nyon--whosonfirst:locality:101914095',
            lat: 46.38641,
            lon: 6.23562,
          },
        },
      },
      {
        label: 'Arnex-sur-Nyon, Switzerland',
        addressId: 'rrPlaceId%3Ape--Arnex-sur-Nyon--whosonfirst%3Alocality%3A1125767805',
        meta: {
          routerank: {
            id: 'pe--Arnex-sur-Nyon--whosonfirst:locality:1125767805',
            lat: 46.3729,
            lon: 6.18989,
          },
        },
      },
    ],
  },
] as unknown as NonEmptyArray<Response>;

export default Response;

// Success
